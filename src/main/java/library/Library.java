...
public class Library {
	...
	/**
	 * Places a hold on book with given call number for the given member
	 * 
	...
	public boolean holdBook(String callNumber, Member member) throws Exception {
		...
		String sqlQuery = "select count(*) as counted from hold where callnumber='" + callNumber + "' and ssn='"
				+ member.getSSN() + "'";
		...
		if (alreadyHeld) {
			...
			sqlQuery = "INSERT INTO Hold VALUES (" + member.getSSN() + ", '" + callNumber + "', CURRENT_DATE)";
	...
	private boolean approveCheckout(int bookId, int memberSSN) throws Exception {
		...
		if (checkedOut) {
			 ...
			 *  holds on this title exceeds the number of available copies.
			 */
	...
	/**
	 * checkMemberHold checks in the member with ssn memberSSN has a hold on book
	 * with id bookId
	 * 
	 ...
	 * @return boolean
	 */
	public boolean checkOutBook(int bookId, int memberSSN, Librarian librarian) throws Exception {
		...
		if (approveCheckout(bookId, memberSSN)) {
			...
			if (!heldByMember) {
				...
				sqlQuery = new String("UPDATE Book SET borrowerssn = " + memberSSN + ", librarianssn = "
						+ librarian.getSSN() + ", duedate = '" + dd + "' " + "WHERE bookid = " + bookId);
				...
				sqlQuery[1] = "UPDATE Book SET borrowerssn = " + memberSSN + ", librarianssn = " + librarian.getSSN()
						+ ", duedate = '" + dd + "' " + "WHERE bookid = " + bookId;
	...
	/**
	 * releaseHold releases a hold on booktitle with call number callNumber for the
	 * specified member.
	 * 
	 ...
	 * @return boolean
	 */
	public boolean releaseHold(String callNumber, Member member) throws Exception {

		String sqlQuery = "DELETE FROM Hold WHERE ssn = " + member.getSSN() + " AND CallNumber = '" + callNumber + "'";

		return myConnection.runUpdate(sqlQuery);
	}

	/**
	 * validateLibrarian returns a librarian object for the librarian with the given
	 * ssn and password
	 * 
	 ...
	 * @return Librarian
	 */
	public Librarian validateLibrarian(String ssn, String passWord) throws Exception {

		String sql = "SELECT * FROM Librarian WHERE ssn = " + ssn + " AND passwd = '" + passWord + "'";
	 ...
	 * @return Member
	 */
	public Member validateMember(String ssn, String passWord) throws Exception {

		String sql = "SELECT * FROM Member WHERE ssn = " + ssn + " AND passwd = '" + passWord + "'";
